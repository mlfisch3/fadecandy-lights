...
3. Check `/etc/fclights/layout.json` matches how the strip is actually split across the Fadecandy's outputs, and that `map` in `/etc/fclights/fcserver.json` covers the same pixel count.
